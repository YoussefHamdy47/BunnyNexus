...
public class CommandLifecycle {
    public static void loadAndRegisterCommands(Config config, BunnyNexus bunnyNexus, CommandRegistry registry) {
        ...
        if (commands.isEmpty()) {
            Logger.debug(() -> "[BunnyNexus] No message commands were loaded/found");
            return;
        }
        ...
        Logger.debug(() -> String.format("[BunnyNexus] Loaded %d message command(s), %d failed to register.", finalSuccess,
                finalFailed));
    }
}
